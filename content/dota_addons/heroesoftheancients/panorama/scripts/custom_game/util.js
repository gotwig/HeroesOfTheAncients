...
function CreateScoreboardFromData(players, callback) {
    for (var key in players) {
		...
		players[key].selectionLockeds = [ players[key].selectionLocked ];
    }
    ...
    for (var key in teams){
        ...
        var player = _.reduce(team, function(p1, p2){
            return {
				...
				selectionLockeds: p1.selectionLockeds.concat(p2.selectionLockeds),
            };
        }, {
            ids: [],
            heroes: [],
        ...
        for (var index in player.names) {
            data.push({ hero: player.heroes[index], name: player.names[index], id: player.ids[index], selectionLocked: player.selectionLockeds[index] });
        }
